...
> **书目信息**：David Flanagan, _JavaScript: The Definitive Guide, Seventh Edition_；李松峰译；机械工业出版社，2021；ISBN 978-7-111-67722-2。原作者前言落款为2020年3月，正文覆盖到ES2020。
>
...
JavaScript没有像组件框架那样固定的挂载、更新、卸载钩子，因此这里把“生命周期”定义为一段程序从被阅读、求值、组织到接入宿主并交付的过程。下面以阶段作三级标题，以原书章节作四级标题，严格按第1章到第17章推进，不把后面的知识提前混入前章。

### 4.1 语言进入运行时：定位、词法、值、求值与控制流

#### 4.1.1 第1章 JavaScript简介：先建立语言与宿主的地图

第1章解决的不是某个语法问题，而是“学习边界在哪里”。作者先用浏览器控制台和Node说明JavaScript既能操作网页，也能在浏览器外运行；随后用一次语言巡礼串起类型、对象、数组、函数、类、模块与异步，并以字符频率柱形图展示这些机制怎样组成真实程序。

字符统计示例的关键不是柱形图外观，而是完整的数据流：读取文本，统一大小写，迭代字符，用`Map`累计频次，排序后输出。下面保留这一思路的最小版本：

```javascript
function characterHistogram(text) {
  ...
  return [...histogram].sort((a, b) => b[1] - a[1]);
}

console.log(characterHistogram("JavaScript"));
```

这段程序提前出现了正则、迭代、映射和排序；按作者在1.5节的建议，第一次看不懂可以跳过，后续章节会自底向上解释每一层。

##### 术语

- **ECMAScript**：标准化语言语法、类型、对象和标准库的规范；JavaScript是这套规范最广泛的实现名称。
- **Host Environment（宿主环境）**：承载JavaScript并提供额外API的环境，例如浏览器提供DOM，Node提供文件与进程API。
- **REPL（Read-Eval-Print Loop，读取-求值-输出循环）**：输入表达式后立即求值并显示结果的交互环境，浏览器控制台和Node都提供类似能力。
- **API（Application Programming Interface，应用程序编程接口）**：一组可由程序调用的约定；必须区分语言标准API和宿主API。

第1章的局限是它有意展示尚未解释的语法，不适合作为逐行背诵材料。正确读法是先形成地图：语言负责计算，宿主负责输入输出，后续章节负责解释地图上的每条路。

#### 4.1.2 第2章 词法结构：源码怎样被读成记号
...
let x = (1)[x].forEach(console.log); // 可能被解析成前一表达式的延续
```

工程上可以选择始终写分号，也可以采用无分号风格，但必须交给格式化器统一，不能依赖“看起来像两句”。严格模式属于第5章的语句内容，后文再处理。
...
#### 4.1.4 第4章 表达式与操作符：值怎样被计算

表达式会求出一个值。第4章从字面量、变量和`this`等主表达式出发，逐步加入对象/数组初始化、函数定义、属性访问、调用、`new`和各类操作符，解决的是“这段源码按什么顺序得到什么结果”。
...
// 先显式完成转换，避免 + 同时承担数值相加和字符串拼接。
const nextCount = Number(input) + 1;
```

`?.`只在左侧为`null`或`undefined`时短路；`??`也只把这两个值视为缺失，因此不会误把`0`、`false`或空字符串换成默认值。操作符的优先级决定结合方式，但复杂表达式应主动加括号或拆成中间变量，让意图不依赖读者背表。

##### 术语

- **Expression（表达式）**：求值后产生一个值的语法片段。
- **Operator（操作符）**：根据一个、两个或三个操作数计算结果的符号或关键字。
- **Precedence（优先级）**：没有括号时，不同操作符的分组顺序。
- **Associativity（结合性）**：同一优先级操作符从左向右或从右向左分组的规则。
- **Short-circuit Evaluation（短路求值）**：结果已确定后不再求值右操作数。
- **Optional Chaining（可选链）**：`?.`安全访问可能为空的属性、元素或调用。
- **Nullish Coalescing（空值合并）**：`??`只为`null`和`undefined`提供默认值。

表达式的局限来自隐式转换与副作用：`==`、`+`和属性访问都可能执行读者没有预期的转换或代码。业务判断通常使用`===`，输入在边界处显式转换，赋值和函数调用不要埋进过长表达式。

一句话理解：表达式负责“算出什么”；先弄清操作数类型，再看优先级与短路规则。

#### 4.1.5 第5章 语句：执行路径怎样分支、重复与失败

语句让程序采取动作。第5章从表达式语句和语句块讲到条件、循环、跳转、异常、`with`、`debugger`、`"use strict"`与声明，解决“下一步执行哪一句”的问题。
...
try {
  ...
  // Error cause是ES2022书后特性，用于保留底层原因。
  throw new SyntaxError("配置不是合法JSON", { cause: error });
}
```

`for/in`枚举对象的可枚举字符串属性，可能包含继承属性；`for/of`消费可迭代对象的值，两者不能混用。异常会沿调用栈传播，只有能恢复、转换或补充上下文的层次才应捕获。模块和类的代码自动处于严格模式；普通脚本可用`"use strict"`指令启用。

##### 术语

- **Statement（语句）**：执行动作或改变控制流的完整单元。
- **Control Flow（控制流）**：条件、循环、跳转和异常共同决定的执行顺序。
- **Iteration（迭代）**：重复执行一段逻辑并依次处理数据。
- **Exception Propagation（异常传播）**：当前层未捕获的异常沿调用栈向上传递，直到处理程序或宿主。
- **Strict Mode（严格模式）**：ES5引入的受限执行模式，禁止部分历史缺陷并把某些静默失败改成异常。
- **Directive Prologue（指令序言）**：脚本或函数体开头的一组字符串表达式语句，`"use strict"`是标准化指令。

复杂判断应拆成有名字的函数，异常不应代替普通分支。`with`会让名字解析不可预测且在严格模式中被禁用，新代码不要使用。循环必须明确终止条件，处理外部数据时还要限制迭代规模。

一句话理解：语句负责“下一步做什么”，异常负责“正常路线走不通时由哪一层处理”。

### 4.2 构造程序结构：对象、数组、函数、类与模块

#### 4.2.1 第6章 对象：用属性和原型组织具名数据

##### 对象模型的核心

对象是属性的无序集合，每个对象都关联一个原型。查询自有属性失败后，会沿原型链查找继承属性。第6章依次讲创建、查询、设置、删除、检测、枚举、扩展和序列化，把对象从字面量语法还原为一套精确的属性规则。
...
Object.prototype.hasOwnProperty.call(point, "x"); // true
"describe" in point; // true，包含继承属性
```

【书后补充】现代运行时可用`Object.hasOwn(point, "x")`替代较长的安全调用。复制对象时要警惕浅复制：展开语法和`Object.assign()`只复制一层可枚举自有属性；访问器会被取值，嵌套对象仍共享引用。JSON只表达有限数据模型，不能保留函数、原型、`undefined`、符号、循环引用或完整数值语义。
...
不要为了复用代码随意修改内置对象的原型。面对不可信JSON，解析成功也不代表结构符合业务约束，仍要校验字段、类型和范围。

一句话理解：对象是按名字取物的柜子，原型是柜子找不到时会继续查询的公共说明书。

#### 4.2.2 第7章 数组：处理有序数据与索引语义

数组是为有序、整数索引数据提供特殊行为的对象。`length`不是普通计数缓存：写入更大的索引会增大它，缩短它则会删除超出范围的元素。第7章还特别区分稀疏槽位和显式的`undefined`。

```javascript
const values = [1, 2, 3];
const doubled = values.filter((value) => value > 1).map((value) => value * 2); // [4, 6]

const sparse = [1, , 3];
1 in sparse; // false：索引1不存在
sparse[1] === undefined; // true：读取结果仍是undefined
```

优先用`map()`、`filter()`、`reduce()`、`find()`和`some()`等方法表达变换意图；需要提前退出或执行异步流程时，普通循环通常更清楚。不要用`for/in`遍历数组，因为它枚举属性名而不是保证只处理数组元素。

##### 术语

- **Array Index（数组索引）**：符合特定范围的非负整数属性名，会与`length`联动。
- **Sparse Array（稀疏数组）**：某些索引根本不存在的数组；空槽不等同于值为`undefined`的元素。
- **Array-like Object（类数组对象）**：有整数属性和`length`，但不一定继承数组方法的对象。
- **Mutator Method（变异方法）**：会原地修改数组的方法，例如`push()`、`splice()`和`sort()`。
- **Higher-order Array Method（高阶数组方法）**：接收回调来执行映射、筛选或归约的方法。
...
一句话理解：数组是按顺序取物的货架；空槽代表货位不存在，`undefined`则代表货位存在但内容是这个特殊值。

#### 4.2.3 第8章 函数：封装行为、调用上下文与闭包
...
### 4.3 处理协议与时间：标准库、迭代、异步与元编程

#### 4.3.1 第11章 JavaScript标准库：复用通用数据结构和协议
...
#### 4.3.3 第13章 异步JavaScript：等待而不阻塞
...
### 4.4 接入现实并交付：浏览器、Node与工程工具

#### 4.4.1 第15章 浏览器中的JavaScript：加载、事件、DOM与网络
...
#### 4.4.2 第16章 Node服务器端JavaScript：事件、流、文件与进程
...
await pipeline(createReadStream("access.log"), createGzip(), process.stdout);
```
...
#### 4.4.3 第17章 JavaScript工具和扩展：检查、测试、打包与转译
