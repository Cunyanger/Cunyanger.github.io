...
* Linux 使用者管理与 Linux 系统管理员”；

第四版加入了 CentOS 7.x、GPT、XFS、systemd、日志格式更新、grub2 和 `nmcli` 等内容。
...
| 比较对象 | 核心管理模型 | Linux 的相对优势 | 需要付出的代价 |
| --- | --- | --- | --- |
...
## 三、沿一台 Linux 主机的生命周期重组技术点

### 阶段一：从硬件需求到可登录系统（第 0～4 章）

#### 为什么安装之前必须先规划

第 0 章先讲硬件，是为了让后续的容量、性能和兼容性判断有依据。CPU 架构决定可运行的机器码，内存保存执行中的程序与数据，磁盘负责持久化；操作系统则管理硬件并为应用提供接口。

指令集（Instruction Set Architecture, ISA）：不同架构的CPU有不同的指令集

精简指令集 （Reduced Instruction Set Computer, RISC）：IBM 公司的 Power Architecture （包括 PowerPC） 系列、与安谋公司 （ARM Holdings） 的 ARM CPU 系列等。它的机器指令就像微指令一样精简

复杂指令集（Complex Instruction Set Computer, CISC）：AMD、 Intel、VIA等的x86架构的CPU，复杂指令集通常由微指令集构成，但精简指令集通常不是
...
- **传统架构**：早期芯片组分为北桥（负责CPU、内存、显卡等高速设备）与南桥（负责硬盘、USB等低速设备）。
- **现代架构**：为提升CPU与内存间的通信速度并节省带宽，主流架构已将内存控制器整合至CPU内部，主板上不再设有独立的北桥芯片。

::: info CPU的工作频率
 影响CPU性能的除了架构还有频率、高速缓存等等

为了让CPU和主板上的其他部件能以相同频率稳定协同工作，主板上有一个统一的基准时钟（BCLK），作为**基础频率**也就是**外频**。当CPU与外部设备（如主板总线）交互时，需要参考这个基准节奏。通常由主板上的晶振器来“喊口号”

由于CPU处理数据的速度要求极高，如果只跟着外部节奏走就太慢了。因此引入了**倍频**，让CPU内部的工作频率可以成倍地高于外部基准频率。

因此，**CPU的主频 = 外频 × 倍频**。

可以想象成跳大绳，倍频就是在大绳到之前多跳了几次，依然与其他部件一致不会绊绳。

超频指的是： 将CPU的倍频或者是外频通过主板的设置功能更改成较高频率的一种方式。但因为CPU的倍频通常在出厂时已经被锁定
而无法修改， 因此较常被超频的为外频。超外频会影响更多部件，因此也会影响稳定性

传统系统架构北桥是“交通要道”，导致其成为系统性能的瓶颈。为了解决这个问题，新的 CPU 设计中， 已经将内存控制器整合到 CPU 内部。目前各个部件已经实现“各自为政”，不受100MHz的限制

::: 

第 2 章因此要求先回答主机用途：是桌面、文件服务器还是高负载计算节点？服务不同，CPU、内存、磁盘吞吐、网络和备份需求就不同。
